...
%
% 1. `blocksPerCondition`
% 2. `blockLength`
% 3. `blocksPerSession`
%
% If `blocksPerCondition` is set, it will determine the generation; consistency
% warnings will occur if the resulting block length isn't equal to
% `blockLength` [*]. If the trials cannot be evenly divided into `numBlocks`
% blocks and there's a remainder of `m` trials, the first `m` blocks will have
% one extra trial.
%
...
%
% If neither `blocksPerCondition` nor `blockLength` are existing fields in
% `blockConfig.task`, the script will set `blocksPerCondition` to
% `blocksPerSession`. If none of the properties are set, the function stops
% with an error.
%
...
if isfield(blockConfig, 'task')
  ...
  if isfield(blockConfig.task, 'blocksPerCondition')
    configBlocksPerCondition = blockConfig.task.blocksPerCondition;
  else
    configBlocksPerCondition = false;
  end
else
...
if ~configBlockLength && ~configBlocksPerSession && ~configBlocksPerCondition
...
if ~configBlocksPerCondition && ~configBlockLength
  if configBlocksPerSession
    configBlocksPerCondition = configBlocksPerSession;
  end
end

% To debug, print the configBlocksPerCondition and blockLength available to the script:
% fprintf('configBlocksPerCondition:\t%d\nblockLength:\t%d\n', ...
%   configBlocksPerSession, configBlockLength);

% 1. get the block length and block number
% 1a. If there was no way to derive numBlocks, use blockLength
addBlock = false;
if ~configBlocksPerCondition 
  ...
  blocksPerCondition = numTrials / blockLength;
  ...
  if lastBlockLength > 0
    ...
    warning('PTF:getBlockLengths:extraBlockNeeded', ...
      ['blocksPerCondition = numTrials / blockLength has a non-zero ', ...
      'remainder; adding an extra block with %d trials to accommodate. To ', ...
      'constrain the number of blocks, set blockConfig.task.blocksPerCondition.'], ...
      lastBlockLength);
    blocksPerCondition = floor(blocksPerCondition) + 1;
  end
else % 1b. Use numBlocks to derive blockLength
  blocksPerCondition = configBlocksPerCondition;
  blockLength = numTrials / blocksPerCondition;

  % If there is no way to create even-length blocks, warn, then distribute the
  % remainder across blocks
  leftover = rem(numTrials, blocksPerCondition);
  if leftover > 0
    warning('PTF:getBlockLengths:unevenBlockLengths', ...
      ['blockLength = numTrials / blocksPerCondition has a non-zero remainder;', ...
  ...
  if configBlockLength && (blockLength + numCatchTrialsPerBlock) ~= configBlockLength
    warning('PTF:getBlockLengths:configBlockLengthInconsistency', ...
      ['blocksPerCondition determined block length, but the generated ', ...
...
blockLengths = blockLength * ones(blocksPerCondition, 1);
