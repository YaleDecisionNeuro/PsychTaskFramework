...
function drawPayoffImageAndLabel(payoffs)
  ...
  [ topLabel, topLabelDims ] = textLookup(payoffs(1), lookupTbl, windowPtr);
  [ botLabel, botLabelDims ] = textLookup(payoffs(2), lookupTbl, windowPtr);

  topLabelXY = [W/2 - topLabelDims(1)/2, Y1 - topLabelDims(2)/2] ...
    + offCenterByPx;
  botLabelXY = [W/2 - botLabelDims(1)/2, Y3 + botLabelDims(2) + padding] ...
    + offCenterByPx;
  ...
  [ topImg, topImgDims ] = imgLookup(payoffs(1), imgLookupTbl, textureBank);
  [ botImg, botImgDims ] = imgLookup(payoffs(2), imgLookupTbl, textureBank);

  bottomXY = [screenCenter - botImgDims(1)/2, ...
              Y3 + botLabelDims(2) + 2 * padding];
  ...
  topXY = [screenCenter - topImgDims(1)/2, ...
           Y1 - topImgDims(2) - topLabelDims(2) - 2 * padding];
...
function drawPayoffAmount(payoffs)
  ...
  topXY = [W/2 - topPayoffDims(1)/2, ...
           Y1 - topPayoffDims(2)/2] + offCenterByPx;
  botXY = [W/2 - botPayoffDims(1)/2, ...
           Y3 + botPayoffDims(2) + padding] + offCenterByPx;
